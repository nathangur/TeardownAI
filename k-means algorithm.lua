function load_data(npoints, radius)
...
function print_eps(data, nclusters, centers, cluster)
    local WIDTH  = 400
    local HEIGHT = 400
...
function kmeans(data, nclusters, init)
    -- K-means Clustering
    --
    assert(nclusters > 0)
  ...
  local N_POINTS   = 10  -- number of points
  local N_CLUSTERS = 2      -- number of clusters
