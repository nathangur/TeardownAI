			...
			if point == nil then
				for j=rg.lastBlank, #points do
					if points[j].color == rg.NO_COLOR then
						...
						--make an if statement to check if points[i].color is in the validSurfaceColours array
						--[[]]
						for v=1, #validSurfaceColours do
							if points.color == validSurfaceColours[1][1] or validSurfaceColours[1][2] or validSurfaceColours[1][3] then
								rg.processRegion[points.color] = {}
								rg.processRegion[points.color][#rg.processRegion[points.color] + 1] = j
								rg.lastBlank = j
							else
								point.color = rg.NO_COLOR
								rg.colorCount = rg.colorCount - 1
								break
							end
						end
